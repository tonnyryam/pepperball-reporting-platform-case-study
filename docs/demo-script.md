...
Use the visible page for the one-minute story: delivered reports, human workflow, the delivered machine-visibility extension, the functional read-only Reporting Assistant, future expansion boundaries, and system controls. Then open only the optional path that matches the audience: sample report, architecture, impact assumptions, or validation evidence. Do not tour every dropdown or repeat artifact links.
...
## Optional technical path - 60 seconds

If the audience asks for implementation depth:
...
5. Show the one-writer release and exact rollback boundary.

If the audience asks for report evidence instead, open the workbook and trace one source-lineage row plus one exception case. Do not also narrate the full reliability sequence.
