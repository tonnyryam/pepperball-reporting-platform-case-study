...
def build(path,long=False):
 ...
 story += [p('Impact with clear boundaries','H1X'),p('<b>Measured:</b> 94.6% fewer report-replay processing rows and 86.6% smaller local PostgreSQL fixture footprint.'),p('<b>Estimated or modeled:</b> about 90% faster retrieval, about 87% storage-cost reduction under linear scaling, 30 hours/week and about $52K annual labor-value opportunity, and up to 227,500 widgets/about $205K annual contribution-margin opportunity under documented assumptions.'),p('The platform supports decisions. These figures are not blanket accuracy, booked savings, realized margin, autonomous decisions, or guaranteed outcomes.')]
 if long:
  story += [PageBreak(),p('Architecture and reliability evidence','TitleX'),p('SharePoint and Forms intake','H2X'),p('Five Power Automate flows cover reminders, machine events, machine whiteboard submissions, molding-report archive, and labor notes. SharePoint lists and libraries retain operational inputs and outputs.'),p('Source-linked reporting core','H2X'),p('Python ingestion and reconciliation normalize evidence into eight PostgreSQL serving tables. Workbooks and the API/SPFx dashboard expose prepared reporting views with source lineage.'),p('Fail-closed delivery','H2X'),p('Validation preserves missing-versus-zero semantics. Artifact readback, bounded repair, one-writer releases, and rollback prevent unvalidated output from silently replacing the last known-good release.'),p('Audience paths','H1X'),p('<b>Supervisors and managers:</b> begin with output use, exceptions, and operating decisions. <b>Executives and recruiters:</b> review the concise impact and ownership narrative. <b>Engineers and successors:</b> follow the architecture, reliability, evidence, operating guide, and private dependency map.'),p('Public/private boundary','H1X'),p('Public assets use synthetic data and sanitized architecture. Tenant identifiers, employee data, production URLs, credentials, and exact operating details remain in the private handoff. Credentials are transferred only through an approved secure channel.')]
